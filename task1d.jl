...
plot!(κs, fittedEv, lc="red", label=L"$\kappa_{min}=%$(κs[findmin(fittedEv)[2]])$, $E_{min}=%$(round(minimum(fittedEv), digits=5))$")
title!("Energy dependence on κ")
xlabel!(L"κ")
ylabel!(L"E_L")

p2 = scatter(κs, stds, label="", mc="darkblue")
plot!(κs,fittedStds, color="red", label=L"$\kappa_{min}=%$(κs[findmin(fittedStds)[2]])$, $\sigma_{min}=%$(round(minimum(fittedStds), digits=5))$")
