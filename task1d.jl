...
plot!(κs, fittedEv, lc="red", label="4th order fit with min at κ=$(κs[findmin(fittedEv)[2]])")
title!("Energy dependence on κ")
xlabel!(L"κ")
ylabel!(L"E_L")

p2 = scatter(κs, stds, label="", mc="darkblue")
plot!(κs,fittedStds, color="red", label="4th order fit with min at κ=$(κs[findmin(fittedStds)[2]])")
