...
function calculatePositions(walkers)
    ...
    for (i,w) in enumerate(walkers)
        pos_list[i] = norm(w.r₁) 
        pos_list[i+length(walkers)] =  norm(w.r₂)
    end
...
@. model(x, p) = 0.5 * gaussian(x, p[1], p[2]) + 0.5 * gaussian(x, p[3], p[4])
...
(μ0, σ0, μ1, σ1) = coef(fit_model)
plot!(x_fit, model(x_fit, coef(fit_model)), label="Two Gaussian Fit: μ₁=$(round(μ0, digits=2)), σ₁=$(round(σ0, digits=2)), μ₂=$(round(μ1, digits=2)), σ₁=$(round(σ1, digits=2))")
